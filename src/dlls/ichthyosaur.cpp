...
#define EMIT_ICKY_SOUND( chan, array ) \
	EMIT_SOUND_DYN ( ENT(pev), chan , array [ RANDOM_LONG(0,VARRAYSIZE( array )-1) ], 1.0, 0.6, 0, RANDOM_LONG(95,105) );
...
float CIchthyosaur::VectorToPitch( const Vector &vec )
{
	...
	{
		pitch = static_cast<float>(atan2(vec.z, sqrt(vec.x*vec.x+vec.y*vec.y)) * 180. / M_PI);
		if (pitch < 0)
			pitch += 360;
	}
	return pitch;
