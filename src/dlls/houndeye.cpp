...
#define	HOUNDEYE_MAX_ATTACK_RADIUS		384.f
...
BOOL CHoundeye :: FValidateHintType ( short sHint )
{
	...
	for ( i = 0 ; i < VARRAYSIZE ( sHoundHints ) ; i++ )
	{
...
void CHoundeye :: SonicAttack ( void )
{
	...
	MESSAGE_BEGIN( MSG_PAS, SVC_TEMPENTITY, pev->origin );
		...
		WRITE_COORD( pev->origin.x);
		WRITE_COORD( pev->origin.y);
		WRITE_COORD( pev->origin.z + 16 + ( HOUNDEYE_MAX_ATTACK_RADIUS / 2.f ) / .2f); // reach damage radius over .3 seconds
		WRITE_SHORT( m_iSpriteTexture );
		WRITE_BYTE( 0 ); // startframe
		WRITE_BYTE( 0 ); // framerate
		WRITE_BYTE( 2 ); // life
	...
	case TASK_SPECIAL_ATTACK1:
		{
			...
			MESSAGE_BEGIN( MSG_PAS, SVC_TEMPENTITY, pev->origin );
				...
				WRITE_COORD( pev->origin.y);
				WRITE_COORD( pev->origin.z + 16);
				WRITE_BYTE( static_cast<int>(50.f * life + 100.f));  // radius
				WRITE_BYTE( static_cast<int>(pev->frame / 25.0f) ); // count
				WRITE_BYTE( static_cast<int>(life * 10.f) ); // life
			MESSAGE_END();
...
{
	{
		...
		VARRAYSIZE ( tlHoundSpecialAttack1 ),
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE		|
...
{
	{
		...
		VARRAYSIZE ( tlHoundCombatFailPVS ),
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
		0,
...
{
	{
		...
		VARRAYSIZE ( tlHoundCombatFailNoPVS ),
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
		0,
