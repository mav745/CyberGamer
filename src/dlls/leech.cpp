...
#define		LEECH_ACCELERATE		10.f
#define		LEECH_CHECK_DIST		45.f
#define		LEECH_SWIM_SPEED		50.f
#define		LEECH_SWIM_ACCEL		80.f
#define		LEECH_SWIM_DECEL		10.f
#define		LEECH_TURN_RATE			90.f
#define		LEECH_SIZEX				10.f
...
void CLeech::SwitchLeechState( void )
{
	...
	else
	{
		...
		if ( pEnemy && pEnemy->pev->waterlevel != 0 )
		{
			...
			m_stateTime = gpGlobals->time + RANDOM_FLOAT( 18.f, 25.f );
			AlertSound();
		}
	}
}
...
void CLeech::AttackSound( void )
{
	...
	{
		EMIT_SOUND_DYN ( ENT(pev), CHAN_VOICE, pAttackSounds[ RANDOM_LONG(0,VARRAYSIZE(pAttackSounds)-1) ], 1.0, ATTN_NORM, 0, PITCH_NORM );
...
{
	EMIT_SOUND_DYN ( ENT(pev), CHAN_VOICE, pAlertSounds[ RANDOM_LONG(0,VARRAYSIZE(pAlertSounds)-1) ], 1.0, ATTN_NORM * 0.5, 0, PITCH_NORM );
}
...
void CLeech::SwimThink( void )
{
	...
	{
		pev->nextthink = gpGlobals->time + RANDOM_FLOAT(1.f,1.5f);
		pev->velocity = g_vecZero;
		return;
	}
	else
	...
	default:
		if ( m_zTime < gpGlobals->time )
		{
			...
			m_zTime = gpGlobals->time + RANDOM_FLOAT( 1.f, 4.f );
		}
		if ( RANDOM_LONG( 0, 100 ) < 10 )
			targetYaw = static_cast<float>(RANDOM_LONG( -30, 30 ));
		pTarget = NULL;
		// oldorigin test
...
void CLeech::Killed(entvars_t *pevAttacker, int iGib)
{
	...
	if ( pev->waterlevel )
	{
		...
		if ( RANDOM_LONG( 0, 99 ) < 70 )
			pev->avelocity.y = static_cast<float>(RANDOM_LONG( -720, 720 ));
