...
void CBaseButton::KeyValue( KeyValueData *pkvd )
{
	...
	{
		m_bLockedSound = static_cast<BYTE>(atof(pkvd->szValue));
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "locked_sentence"))
	{
		m_bLockedSentence = static_cast<BYTE>(atof(pkvd->szValue));
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "unlocked_sound"))
	{
		m_bUnlockedSound = static_cast<BYTE>(atof(pkvd->szValue));
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "unlocked_sentence"))
	{
		m_bUnlockedSentence = static_cast<BYTE>(atof(pkvd->szValue));
		pkvd->fHandled = TRUE;
	}
...
{
	SetThink ( &CBaseButton::ButtonSpark );
	pev->nextthink = gpGlobals->time + ( 0.1f + RANDOM_FLOAT ( 0.f, 1.5f ) );// spark again at random interval
...
void CEnvSpark::Spawn(void)
{
	...
	pev->nextthink = gpGlobals->time + ( 0.1f + RANDOM_FLOAT ( 0.f, 1.5f ) );
...
{
	pev->nextthink = gpGlobals->time + 0.1f + RANDOM_FLOAT (0.f, m_flDelay);
