...
#define		AGRUNT_MELEE_DIST	100.f
...
void CAGrunt :: PrescheduleThink ( void )
{
	if ( ShouldSpeak() )
	{
		if ( m_flNextWordTime < gpGlobals->time )
		{
			...
			do
			{
				num = RANDOM_LONG(0,VARRAYSIZE(pIdleSounds)-1);
...
void CAGrunt :: DeathSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pDieSounds[RANDOM_LONG(0,VARRAYSIZE(pDieSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AlertSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAlertSounds[RANDOM_LONG(0,VARRAYSIZE(pAlertSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AttackSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0,VARRAYSIZE(pAttackSounds)-1)], 1.0f, ATTN_NORM );
}
...
void CAGrunt :: PainSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0,VARRAYSIZE(pPainSounds)-1)], 1.0f, ATTN_NORM );
}
...
void CAGrunt :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case AGRUNT_AE_LEFT_PUNCH:
		{
			CBaseEntity *pHurt = CheckTraceHullAttack( AGRUNT_MELEE_DIST, static_cast<int>(gSkillData.agruntDmgPunch), DMG_CLUB );
...
void CAGrunt :: Precache()
{
	...
	for ( i = 0; i < VARRAYSIZE( pDieSounds ); i++ )
...
{
	{
		...
		VARRAYSIZE ( tlAGruntSuppressHornet ),
		0,
		0,
...
{
	{
		...
		VARRAYSIZE ( tlAGruntRangeAttack1 ),
		bits_COND_NEW_ENEMY			|
...
{
	{
		...
		VARRAYSIZE ( tlAGruntHiddenRangeAttack1 ),
		bits_COND_NEW_ENEMY			|
		bits_COND_HEAVY_DAMAGE		|
		bits_COND_HEAR_SOUND,
...
{
	{
		...
		VARRAYSIZE ( tlAGruntTakeCoverFromEnemy ),
		bits_COND_NEW_ENEMY,
		0,
...
{
	{
		...
		VARRAYSIZE ( tlAGruntVictoryDance ),
		bits_COND_NEW_ENEMY		|
...
{
	{
		...
		VARRAYSIZE ( tlAGruntThreatDisplay ),
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
