...
void COsprey::Flight( )
{
	...
	float m_flIdealtilt = (160.f - flSpeed) / 10.0f;
	...
	else
	{
		...
		if (pPlayer)
		{
			...
			{
				m_iPitch = (int)pitch;
				EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor4.wav", 1.0f, 0.15f, SND_CHANGE_PITCH | SND_CHANGE_VOL, (int)pitch);
				// ALERT( at_console, "%.0f\n", pitch );
			}
		}
...
void COsprey::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if (ptr->iHitgroup == 3)
	{
		...
		m_iDoLeftSmokePuff = 3 + static_cast<int>(flDamage / 5.0f);
	}
	...
	if (ptr->iHitgroup == 2)
	{
		...
		m_iDoRightSmokePuff = 3 + static_cast<int>(flDamage / 5.0f);
	}
