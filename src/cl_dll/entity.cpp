	...
	void	( *Callback_TempEntPlaySound )( TEMPENTITY *pTemp, float damp ) )
{
	...
	while ( pTemp )
	{
		...
		life = pTemp->die - static_cast<float>(client_time);
		pnext = pTemp->next;
		if ( life < 0 )
		{
			if ( pTemp->flags & FTENT_FADEOUT )
			{
				...
				pTemp->entity.curstate.renderamt = static_cast<int>( pTemp->entity.baseline.renderamt * (1.f + life * pTemp->fadeSpeed) );
		...
		else
		{
			...
			{
				pTemp->x += pTemp->entity.baseline.origin[0] * static_cast<float>(frametime);
				pTemp->y += pTemp->entity.baseline.origin[1] * static_cast<float>(frametime);

				pTemp->entity.origin[0] = pTemp->x + sinf( pTemp->entity.baseline.origin[2] + static_cast<float>(client_time) * pTemp->entity.prevstate.frame ) * (10*pTemp->entity.curstate.framerate);
			...
			else if ( pTemp->flags & FTENT_SPIRAL )
			{
				...
				pTemp->entity.origin[0] += static_cast<float>(pTemp->entity.baseline.origin[0] * frametime + 8. * sin( client_time * 20 + reinterpret_cast<int>(pTemp) ));
				pTemp->entity.origin[1] += static_cast<float>(pTemp->entity.baseline.origin[1] * frametime + 4. * sin( client_time * 30 + reinterpret_cast<int>(pTemp) ));
				pTemp->entity.origin[2] += static_cast<float>(pTemp->entity.baseline.origin[2] * frametime);
			}

			else
			{
				for ( i = 0; i < 3; i++ )
					pTemp->entity.origin[i] += static_cast<float>(pTemp->entity.baseline.origin[i] * frametime);
			}
			...
			if ( pTemp->flags & FTENT_SPRANIMATE )
			{
				...
				{
					pTemp->entity.curstate.frame -= floorf(pTemp->entity.curstate.frame);
...
// Experiment
			...
			{
				pTemp->entity.angles[0] += static_cast<float>(pTemp->entity.baseline.angles[0] * frametime);
				pTemp->entity.angles[1] += static_cast<float>(pTemp->entity.baseline.angles[1] * frametime);
				pTemp->entity.angles[2] += static_cast<float>(pTemp->entity.baseline.angles[2] * frametime);
			...
			if ( pTemp->flags & (FTENT_COLLIDEALL | FTENT_COLLIDEWORLD) )
			{
				...
				if ( traceFraction != 1 )	// Decent collision now, and damping works
				{
					...
					VectorMA( pTemp->entity.prevstate.origin, static_cast<float>(traceFraction*frametime), pTemp->entity.baseline.origin, pTemp->entity.origin );
					// Damp velocity
