...
static int R_RankForRenderMode( const cl_entity_t *ent )
{
...
===============
*/
//static int R_SolidEntityCompare( const void *a, const void *b )
//{
//	const cl_entity_t	*ent1, *ent2;

//	ent1 = reinterpret_cast<const cl_entity_t*>(*reinterpret_cast<const void**>(reinterpret_cast<int>(a)));
//	ent2 = reinterpret_cast<const cl_entity_t*>(*reinterpret_cast<const void**>(reinterpret_cast<int>(b)));
//	//ent1 = reinterpret_cast<const cl_entity_t*>(a);
//	//ent2 = reinterpret_cast<const cl_entity_t*>(b);
...
static void R_SetupFrame( void )
{
	...
	qsort( tr.solid_entities, tr.num_solid_entities, sizeof( cl_entity_t* ), reinterpret_cast<int (*)(const void*,const void*)>(R_SolidEntityCompare) );

	if( !gl_nosort->integer )
	{
		// sort translucents entities by rendermode and distance
		qsort( tr.trans_entities, tr.num_trans_entities, sizeof( cl_entity_t* ), reinterpret_cast<int (*)(const void*,const void*)>(R_TransEntityCompare) );
	}
...
//	if( RI.params & RP_MIRRORVIEW ) RI.projectionMatrix[0][0] = -RI.projectionMatrix[0][0];
	...
	if( RP_NORMALPASS( ))
	{
		...
		x = (int)floorf( static_cast<float>(RI.viewport[0]) * glState.width / glState.width );
		x2 = (int)ceilf(static_cast<float>( RI.viewport[0] + RI.viewport[2] ) * glState.width / glState.width );
		y = (int)floorf( glState.height - static_cast<float>(RI.viewport[1]) * glState.height / glState.height );
		y2 = (int)ceilf( glState.height - static_cast<float>( RI.viewport[1] + RI.viewport[3] ) * glState.height / glState.height );
...
static render_api_t gRenderAPI =
{
	...
	reinterpret_cast<int		(*)( const char *name, int width, int height, const void *buffer, int flags )>(GL_CreateTexture),
	GL_SetTextureType,
	...
	reinterpret_cast<void*(*)( const char *filename, int ignore_hwgamma )>(AVI_LoadVideoNoSound),
	reinterpret_cast<int(*)( void *Avi, long *xres, long *yres, float *duration )>(AVI_GetVideoInfo),
	reinterpret_cast<long(*)( void *Avi, float time )>(AVI_GetVideoFrameNumber),
	reinterpret_cast<byte*(*)( void *Avi, long frame )>(AVI_GetVideoFrame),
	R_UploadStretchRaw,
	reinterpret_cast<void(*)( void *Avi )>(AVI_FreeVideo),
	reinterpret_cast<int(*)( void *Avi )>(AVI_IsActive),
	GL_Bind,
	GL_SelectTexture,
	GL_LoadTexMatrixExt,
