...
static int CL_AdjustXPos( float x, int width, int totalWidth )
{
	...
	if( x == -1 )
	{
		xPos = (int)(( clgame.scrInfo.iWidth - width ) * 0.5f);
	}
	else
	{
		if ( x < 0 )
			xPos = (int)((1.0f + x) * clgame.scrInfo.iWidth) - totalWidth;	// Alight right
		else // align left
			xPos = (int)(x * clgame.scrInfo.iWidth);
	}
...
static int CL_AdjustYPos( float y, int height )
{
	...
	if( y == -1 ) // centered?
	{
		yPos = (int)(( clgame.scrInfo.iHeight - height ) * 0.5f);
	}
	else
	{
		// Alight bottom?
		if( y < 0 )
			yPos = (int)((1.0f + y) * clgame.scrInfo.iHeight) - height; // Alight bottom
		else // align top
			yPos = (int)(y * clgame.scrInfo.iHeight);
	}
...
static void CL_InitTitles( const char *filename )
{
	...
	pMemFile = FS_LoadFile( filename, reinterpret_cast<fs_offset_t*>(&fileSize), false );
	if( !pMemFile ) return;
...
void CL_DrawCrosshair( void )
{
	...
	if( !VectorIsNull( cl.refdef.crosshairangle ))
	{
		...
		x += (int)(0.5f * screen[0] * scr_width->integer + 0.5f);
		y += (int)(0.5f * screen[1] * scr_height->integer + 0.5f);
	}
	...
	*(int *)clgame.ds.spriteColor = *(int *)clgame.ds.rgbaCrosshair;
	...
	SPR_EnableScissor( (int)(x - 0.5f * width), (int)(y - 0.5f * height), width, height );
...
void CL_DrawHUD( int state )
{
	...
	case CL_ACTIVE:
		...
		CL_DrawCrosshair ();
		CL_DrawCenterPrint ();
		clgame.dllFuncs.pfnRedraw( (float)cl.time, cl.refdef.intermission );
		break;
	case CL_PAUSED:
		CL_DrawScreenFade ();
		CL_DrawCrosshair ();
...
void CL_InitEdicts( void )
{
	...
	cls.packet_entities = reinterpret_cast<entity_state_t*>(Z_Realloc( cls.packet_entities, sizeof( entity_state_t ) * cls.num_client_entities ));
	clgame.entities = reinterpret_cast<cl_entity_t*>(Mem_Alloc( clgame.mempool, sizeof( cl_entity_t ) * clgame.maxEntities ));
	clgame.static_entities = reinterpret_cast<cl_entity_t*>(Mem_Alloc( clgame.mempool, sizeof( cl_entity_t ) * MAX_STATIC_ENTITIES ));
	clgame.numStatics = 0;
...
static qboolean CL_LoadHudSprite( const char *szSpriteName, model_t *m_pSprite, qboolean mapSprite, uint texFlags )
{
	...
	buf = FS_LoadFile( szSpriteName, reinterpret_cast<fs_offset_t*>(&size), false );
	if( !buf ) return false;
...
static client_sprite_t *pfnSPR_GetList( char *psz, int *piCount )
{
	...
	afile = reinterpret_cast<char*>(FS_LoadFile( psz, NULL, false ));
	...
	pList = reinterpret_cast<client_sprite_t*>(Mem_Alloc( pool, sizeof( client_sprite_t ) * numSprites ));
...
int TriSpriteTexture( model_t *pSpriteModel, int frame )
{
	...
	psprite = reinterpret_cast<msprite_t*>(pSpriteModel->cache.data);
...
void NetAPI_Status( net_status_t *status )
{
	...
	status->packet_loss = (int)(cls.packet_loss / 100.f); // percent
...
triangleapi_t gTriApi =
{
	...
	TriColor4ub,
	TriTexCoord2f,
	reinterpret_cast<void	(*)( float *worldPnt )>(TriVertex3fv),
	TriVertex3f,
	TriBrightness,
	TriCullFace,
	TriSpriteTexture,
	reinterpret_cast<int	(*)( float *world, float *screen )>(R_WorldToScreen),	// NOTE: XPROJECT, YPROJECT should be done in client.dll
	TriFog,
	reinterpret_cast<void	(*)( float *screen, float *world  )>(R_ScreenToWorld),
	TriGetMatrix,
	TriBoxInPVS,
	TriLightAtPoint,
	TriColor4fRendermode,
...
{
	CL_AllocParticle,
	reinterpret_cast<void(*)( float *org )>(CL_BlobExplosion),
	reinterpret_cast<void(*)( float *org, float *dir, int pcolor, int speed )>(CL_Blood),
	reinterpret_cast<void(*)( float *org, int colorindex, int modelIndex, int modelIndex2, float size )>(CL_BloodSprite),
	reinterpret_cast<void(*)( float *org, float *dir, int pcolor, int speed )>(CL_BloodStream),
	reinterpret_cast<void(*)( float *pos, float *size, float *dir, float random, float life, int count, int modelIndex, char flags )>(CL_BreakModel),
	reinterpret_cast<void(*)( float *mins, float *maxs, float height, int modelIndex, int count, float speed )>(CL_Bubbles),
	reinterpret_cast<void(*)( float *start, float *end, float height, int modelIndex, int count, float speed )>(CL_BubbleTrail),
	reinterpret_cast<void(*)( float *pos )>(CL_BulletImpactParticles),
	CL_EntityParticles,
	CL_Explosion,
	CL_FizzEffect,
	CL_FireField,
	reinterpret_cast<void		(*)( float *org )>(CL_FlickerParticles),
	reinterpret_cast<void		(*)( float *org, int modelIndex, int reverse )>(CL_FunnelSprite),
	reinterpret_cast<void		(*)( float *end, float radius, int count, float life )>(CL_Implosion),
	reinterpret_cast<void		(*)( float *org, int reverse )>(CL_Large_Funnel),
	reinterpret_cast<void		(*)( float *org )>(CL_LavaSplash),
	reinterpret_cast<void		(*)( float *org, float *dir, float *noise, int count, int decalCount, int *decalIndices )>(CL_MultiGunshot),
	reinterpret_cast<void		(*)( float *pos1, int type )>(CL_MuzzleFlash),
	reinterpret_cast<void		(*)( float *mins, float *maxs, unsigned char r, unsigned char g, unsigned char b, float life )>(CL_ParticleBox),
	reinterpret_cast<void		(*)( float *pos, int size, int color, float life )>(CL_ParticleBurst),
	reinterpret_cast<void		(*)( float *org )>(CL_ParticleExplosion),
	reinterpret_cast<void		(*)( float *org, int colorStart, int colorLength )>(CL_ParticleExplosion2),
	reinterpret_cast<void		(*)( float *start, float *end, unsigned char r, unsigned char g, unsigned char b, float life )>(CL_ParticleLine),
	CL_PlayerSprites,
	reinterpret_cast<void		(*)( float *origin, float *velocity, int modelIndex, int life, int owner, void (*hitcallback)( struct tempent_s *ent, struct pmtrace_s *ptr ) )>(CL_Projectile),
	reinterpret_cast<void		(*)( float *pos )>(CL_RicochetSound),
	reinterpret_cast<void		(*)( float *pos, struct model_s *pmodel, float duration, float scale )>(CL_RicochetSprite),
	reinterpret_cast<void		(*)( float *pos )>(CL_RocketFlare),
	CL_RocketTrail,
	reinterpret_cast<void		(*)( float *org, float *dir, int color, int count )>(CL_RunParticleEffect),
	reinterpret_cast<void		(*)( float *start, float *end )>(CL_ShowLine),
	reinterpret_cast<void		(*)( float *pos, int count, int velocityMin, int velocityMax )>(CL_SparkEffect),
	reinterpret_cast<void		(*)( float *pos )>(CL_SparkShower),
	reinterpret_cast<void		(*)( float *pos, int count, int velocityMin, int velocityMax )>(CL_SparkStreaks),
	reinterpret_cast<void		(*)( float *pos, float *dir, int modelIndex, int count, int speed, int spread, int rendermode )>(CL_Spray),
	CL_Sprite_Explode,
	CL_Sprite_Smoke,
	reinterpret_cast<void		(*)( float *pos, float *dir, int modelIndex, int count, int speed, int iRand )>(CL_Sprite_Spray),
	reinterpret_cast<void		(*)( int type, float *start, float *end, int modelIndex, int count, float life, float size, float amplitude, int renderamt, float speed )>(CL_Sprite_Trail),
	CL_Sprite_WallPuff,
	reinterpret_cast<void		(*)( float *pos, float *dir, int color, int count, float speed, int velocityMin, int velocityMax )>(CL_StreakSplash),
	reinterpret_cast<void		(*)( float *start, float *end )>(CL_TracerEffect),
	CL_UserTracerParticle,
	CL_TracerParticles,
	reinterpret_cast<void		(*)( float *org )>(CL_TeleportSplash),
	reinterpret_cast<void		(*)( float *pos, float speed, float life, int count, int modelIndex )>(CL_TempSphereModel),
	reinterpret_cast<TEMPENTITY	*(*)( float *pos, float *dir, float *angles, float life, int modelIndex, int soundtype )>(CL_TempModel),
	reinterpret_cast<TEMPENTITY	*(*)( float *pos, int spriteIndex, float framerate )>(CL_DefaultSprite),
	reinterpret_cast<TEMPENTITY	*(*)( float *pos, float *dir, float scale, int modelIndex, int rendermode, int renderfx, float a, float life, int flags )>(CL_TempSprite),
	CL_DecalIndex,
	reinterpret_cast<int		(*)( char *name )>(CL_DecalIndexFromName),
	CL_DecalShoot,
	CL_AttachTentToPlayer,
	CL_KillAttachedTents,
	reinterpret_cast<BEAM		*(*)( int type, float *start, float *end, int modelIndex, float life, float width, float amplitude, float brightness, float speed, int startFrame, float framerate, float r, float g, float b )>(CL_BeamCirclePoints),
	reinterpret_cast<BEAM		*(*)( int startEnt, float *end, int modelIndex, float life, float width, float amplitude, float brightness, float speed, int startFrame, float framerate, float r, float g, float b )>(CL_BeamEntPoint),
	CL_BeamEnts,
	CL_BeamFollow,
	CL_BeamKill,
	reinterpret_cast<BEAM		*(*)( float *start, float *end, int modelIndex, float life, float width, float amplitude, float brightness, float speed )>(CL_BeamLightning),
	reinterpret_cast<BEAM		*(*)( float *start, float *end, int modelIndex, float life, float width, float amplitude, float brightness, float speed, int startFrame, float framerate, float r, float g, float b )>(CL_BeamPoints),
	CL_BeamRing,
	CL_AllocDlight,
	CL_AllocElight,
	reinterpret_cast<TEMPENTITY	*(*)( float *org, struct model_s *model )>(CL_TempEntAlloc),
	reinterpret_cast<TEMPENTITY	*(*)( float *org )>(CL_TempEntAllocNoModel),
	reinterpret_cast<TEMPENTITY	*(*)( float *org, struct model_s *model )>(CL_TempEntAllocHigh),
	reinterpret_cast<TEMPENTITY	*(*)( float *origin, struct model_s *model, int high, void (*callback)( struct tempent_s *ent, float frametime, float currenttime ))>(CL_TempEntAllocCustom),
	CL_GetPackedColor,
	CL_LookupColor,
	CL_DecalRemoveAll,
	CL_FireCustomDecal,
...
static cl_enginefunc_t gEngfuncs =
{
	...
	pfnGetScreenInfo,
	pfnSetCrosshair,
	reinterpret_cast<struct cvar_s *(*)( char *szName, char *szValue, int flags )>(pfnCvar_RegisterVariable),
	reinterpret_cast<float	(*)( char *szName )>(Cvar_VariableValue),
	reinterpret_cast<char*	(*)( char *szName )>(Cvar_VariableString),
	reinterpret_cast<int	(*)( char *cmd_name, void (*function)(void) )>(pfnAddClientCommand),
	reinterpret_cast<int	(*)( char *szMsgName, pfnUserMsgHook pfn )>(pfnHookUserMsg),
	reinterpret_cast<int	(*)( char *szCmdString )>(pfnServerCmd),
	reinterpret_cast<int	(*)( char *szCmdString )>(pfnClientCmd),
	pfnGetPlayerInfo,
	reinterpret_cast<void	(*)( char *szSound, float volume )>(pfnPlaySoundByName),
	pfnPlaySoundByIndex,
	AngleVectors,
	CL_TextMessageGet,
	pfnDrawCharacter,
	...
	pfnSetViewAngles,
	CL_GetMaxClients,
	reinterpret_cast<void	(*)( char *cvar, float value )>(Cvar_SetFloat),
	reinterpret_cast<int       (*)( void )>(Cmd_Argc),
	Cmd_Argv,
	Con_Printf,
	Con_DPrintf,
	Con_NPrintf,
	...
	pfnCalcShake,
	pfnApplyShake,
	reinterpret_cast<int(*)( float *point, int *truecontents )>(pfnPointContents),
	reinterpret_cast<int(*)( float *p )>(CL_WaterEntity),
	pfnTraceLine,
	CL_LoadModel,
	CL_AddEntity,
	CL_GetSpritePointer,
	...
	Com_RandomFloat,
	Com_RandomLong,
	reinterpret_cast<void	(*)( char *name, void ( *pfnEvent )( struct event_args_s *args ))>(pfnHookEvent),
	Con_Visible,
	pfnGetGameDirectory,
	pfnCVarGetPointer,
	Key_LookupBinding,
	...
	reinterpret_cast<byte*	(*)( char *path, int usehunk, int *pLength )>(COM_LoadFile),
	COM_ParseFile,
	COM_FreeFile,
	&gTriApi,
	&gEfxApi,
	...
	pfnSetMouseEnable,
	Cvar_GetList,
	reinterpret_cast<void*(*)( void )>(Cmd_GetFirstFunctionHandle),
	reinterpret_cast<void*(*)( void *cmdhandle )>(Cmd_GetNextFunctionHandle),
	reinterpret_cast<const char*(*)( void *cmdhandle )>(Cmd_GetName),
	pfnGetClientOldTime,
	pfnGetGravity,
	Mod_Handle,
	pfnEnableTexSort,
	...
	pfnVGUI2DrawCharacter,
	pfnVGUI2DrawCharacterAdditive,
	reinterpret_cast<unsigned int(*)( char *filename )>(Sound_GetApproxWavePlayLen),
	GetCareerGameInterface,
	reinterpret_cast<void(*)( char *name, char *value )>(Cvar_Set),
	pfnIsCareerMatch,
	pfnPlaySoundVoiceByName,
	pfnMP3_InitStream,
	Sys_DoubleTime,
...
qboolean CL_LoadProgs( const char *name )
{
	...
	if(( F = reinterpret_cast<CL_EXPORT_FUNCS>(Com_GetProcAddress( clgame.hInstance, "F" ))) != NULL )
	{
