...
{
	*s = &g_nStudioCount;
	*a = reinterpret_cast<int*>(&r_stats.c_studio_models_drawn);
}
...
mstudioanim_t *R_StudioGetAnim( model_t *m_pSubModel, mstudioseqdesc_t *pseqdesc )
{
	...
	if( !Mod_CacheCheck(( cache_user_t *)&( paSequences[pseqdesc->seqgroup] )))
	{
		...
		buf = FS_LoadFile( filepath, reinterpret_cast<fs_offset_t*>(&filesize), false );
...
mstudiotexture_t *R_StudioGetTexture( cl_entity_t *e )
{
	...
	if(( phdr = reinterpret_cast<studiohdr_t*>(Mod_Extradata( e->model ))) == NULL )
		return NULL;
...
//{
//	const sortedmesh_t *ma = reinterpret_cast<const sortedmesh_t*>(a);
...
static void R_StudioDrawPoints( void )
{
	...
	if( r_studio_sort_textures->integer )
	{
		...
		qsort( g_sortedMeshes, m_pSubModel->nummesh, sizeof( sortedmesh_t ), reinterpret_cast<int (*)(const void*,const void*)>(R_StudioMeshCompare) );
	}
			...
			for( ; i > 0; i--, ptricmds += 4 )
			{
				...
				if(!( g_nForceFaceFlags & STUDIO_NF_CHROME ))
					...
					else if( g_iRenderMode == kRenderTransColor )
					{
						color24	*clr;
						clr = &RI.currententity->curstate.rendercolor;
						pglColor4ub( clr->r, clr->g, clr->b, (byte)(alpha * 255) );
					}
...
void R_StudioRenderFinal( void )
{
	...
	else
	{
		...
		{
			R_StudioSetupModel( i, reinterpret_cast<void**>(&m_pBodyPart), reinterpret_cast<void**>(&m_pSubModel) );
...
static void R_StudioLoadTexture( model_t *mod, studiohdr_t *phdr, mstudiotexture_t *ptexture )
{
	...
	if( !Q_strnicmp( ptexture->name, "DM_Base", 7 ) || !Q_strnicmp( ptexture->name, "remap", 5 ))
	{
		...
		tx = reinterpret_cast<texture_t*>(Mem_Alloc( mod->mempool, sizeof( *tx ) + size ));
		mod->textures[i] = tx;
...
void Mod_UnloadStudioModel( model_t *mod )
{
	...
	pstudio = reinterpret_cast<studiohdr_t*>(mod->cache.data);
...
static engine_studio_api_t gStudioAPI =
{
	Mod_Calloc,
	Mod_CacheCheck,
	reinterpret_cast<void		( * )( char *path, struct cache_user_s *cu )>(Mod_LoadCacheFile),
	Mod_ForName,
	Mod_Extradata,
	Mod_Handle,
	pfnGetCurrentEntity,
	...
	R_StudioDrawAbsBBox,
	R_StudioDrawBones,
	reinterpret_cast<void		( * )( void *ptexturehdr, int index )>(R_StudioSetupSkin),
	R_StudioSetRemapColors,
	R_StudioSetupPlayerModel,
	R_StudioClientEvents,
	R_StudioGetForceFaceFlags,
	R_StudioSetForceFaceFlags,
	reinterpret_cast<void		( * )( void *header )>(R_StudioSetHeader),
	R_StudioSetRenderModel,
	R_StudioSetupRenderer,
	R_StudioRestoreRenderer,
	R_StudioSetChromeOrigin,
