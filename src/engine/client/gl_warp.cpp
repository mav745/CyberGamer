...
void R_InitSky( mip_t *mt, texture_t *tx )
{
	...
	trans = reinterpret_cast<uint*>(Mem_Alloc( r_temppool, r_sky->height * r_sky->height * sizeof( *trans )));
	...
	// load it in
	tr.solidskyTexture = GL_LoadTextureInternal( "solid_sky", &r_temp, static_cast<texFlags_t>(TF_UNCOMPRESSED|TF_NOMIPMAP), false );
	...
	// load it in
	tr.alphaskyTexture = GL_LoadTextureInternal( "alpha_sky", &r_temp, static_cast<texFlags_t>(TF_UNCOMPRESSED|TF_NOMIPMAP), false );
