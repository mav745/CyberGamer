...
static qboolean DS_CreateBuffers( void *hInst )
{
	...
	if( DS_OK != pDS->SetCooperativeLevel( reinterpret_cast<HWND>(hInst), DSSCL_EXCLUSIVE ))
	{
	...
	if( pDS->CreateSoundBuffer( &dsbuf, &pDSPBuf, NULL ) == DS_OK )
	{
		pformat = format;
		...
		if( pDSPBuf->SetFormat( &pformat ) != DS_OK )
		{
			if( snd_firsttime )
				MsgDev( D_NOTE, "- failed\n" );
		}
	...
	if( !primary_format_set || !s_primary->integer )
	{
		...
		if( pDS->CreateSoundBuffer( &dsbuf, &pDSBuf, NULL ) == DS_OK )
		{
			MsgDev( D_NOTE, "- ok\n" );
		}
		else
		...
		if( pDSBuf->GetCaps( &dsbcaps ) != DS_OK )
		{
	...
	else
	{
		...
		if( pDS->SetCooperativeLevel( reinterpret_cast<HWND>(hInst), DSSCL_WRITEPRIMARY ) != DS_OK )
		{
			MsgDev( D_NOTE, "- failed\n" );
			SNDDMA_FreeSound ();
			return false;
		...
		if( pDSPBuf->GetCaps( &dsbcaps ) != DS_OK )
		{
			MsgDev( D_ERROR, "DS_CreateBuffers: GetCaps failed\n");
			SNDDMA_FreeSound ();
			return false;
	...
	if( pDSBuf->Play( 0, 0, DSBPLAY_LOOPING ) != DS_OK )
	{
...
static void DS_DestroyBuffers( void )
{
	...
	if( pDS )
	{
		...
		pDS->SetCooperativeLevel( snd_hwnd, DSSCL_NORMAL );
	}
...
void SNDDMA_BeginPainting( void )
{
	...
	while(( hr = pDSBuf->Lock( 0, gSndBufSize, reinterpret_cast<LPVOID*>(&pbuf), &locksize, reinterpret_cast<LPVOID*>(&pbuf2), &dwSize2, 0 )) != DS_OK )
	{
