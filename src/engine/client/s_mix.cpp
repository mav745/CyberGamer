...
void S_MixChannel( channel_t *pChannel, void *pData, int outputOffset, int inputOffset, uint fracRate, int outCount )
{
	...
	{
		if( pSource->width == 1 )
			S_Mix8Mono( pbuf, pvol, reinterpret_cast<byte*>(pData), inputOffset, fracRate, outCount );
		else S_Mix16Mono( pbuf, pvol, (short *)pData, inputOffset, fracRate, outCount );
	}
	else
	{
		if( pSource->width == 1 )
			S_Mix8Stereo( pbuf, pvol, reinterpret_cast<byte*>(pData), inputOffset, fracRate, outCount );
...
int S_MixDataToDevice( channel_t *pChannel, int sampleCount, int outputRate, int outputOffset )
{
	...
	while( sampleCount > 0 )
	{
		...
		availableSamples = S_GetOutputData( pSource, reinterpret_cast<void**>(&pData), (int)pChannel->pMixer.sample, inputSampleCount, use_loop );
