...
qboolean VID_ScreenShot( const char *filename, int shot_type )
{
	...
	r_shot = reinterpret_cast<rgbdata_t*>(Mem_Alloc( r_temppool, sizeof( rgbdata_t )));
	...
	r_shot->palette = NULL;
	r_shot->buffer = reinterpret_cast<byte*>(Mem_Alloc( r_temppool, r_shot->size ));
...
qboolean VID_CubemapShot( const char *base, uint size, const float *vieworg, qboolean skyshot )
{
	...
	// alloc space
	temp = reinterpret_cast<byte*>(Mem_Alloc( r_temppool, size * size * 3 ));
	buffer = reinterpret_cast<byte*>(Mem_Alloc( r_temppool, size * size * 3 * 6 ));
	r_shot = reinterpret_cast<rgbdata_t*>(Mem_Alloc( r_temppool, sizeof( rgbdata_t )));
	r_side = reinterpret_cast<rgbdata_t*>(Mem_Alloc( r_temppool, sizeof( rgbdata_t )));
