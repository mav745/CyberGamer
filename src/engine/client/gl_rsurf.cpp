...
static void SubdividePolygon_r( msurface_t *warpface, int numverts, float *verts )
{
	...
	poly = reinterpret_cast<glpoly_t*>(Mem_Alloc( loadmodel->mempool, sizeof( glpoly_t ) + ((numverts-4)+2) * VERTEXSIZE * sizeof( float )));
...
void GL_BuildPolygonFromSurface( model_t *mod, msurface_t *fa )
{
	...
	// draw texture
	poly = reinterpret_cast<glpoly_t*>(Mem_Alloc( mod->mempool, sizeof( glpoly_t ) + ( lnumverts - 4 ) * VERTEXSIZE * sizeof( float )));
	poly->next = fa->polys;
	poly->flags = fa->flags;
	fa->polys = poly;
	...
	if( !gl_keeptjunctions->integer && !( fa->flags & SURF_UNDERWATER ))
	{
		...
		{
			vec3_t	v1, v2;
			float	*prev, *this1, *next;

			prev = poly->verts[(i + lnumverts - 1) % lnumverts];
			next = poly->verts[(i + 1) % lnumverts];
			this1 = poly->verts[i];

			VectorSubtract( this1, prev, v1 );
			VectorNormalize( v1 );
...
static void LM_UploadBlock( qboolean dynamic )
{
	...
	else
	{
		...
		tr.lightmapTextures[i] = GL_LoadTextureInternal( lmName, &r_lightmap, static_cast<texFlags_t>(TF_FONT), false );
...
//{
//	const msurface_t	*surf1, *surf2;
//	mextrasurf_t	*info1, *info2;
//	vec3_t		org1, org2;
//	float		len1, len2;

//	surf1 = reinterpret_cast<const msurface_t*>(a);
//	surf2 = reinterpret_cast<const msurface_t*>(b);
...
void R_DrawBrushModel( cl_entity_t *e )
{
	...
	if( need_sort && !gl_nosort->integer )
		qsort( world.draw_surfaces, num_sorted, sizeof( msurface_t* ), reinterpret_cast<int (*)(const void*,const void*)>(R_SurfaceCompare) );
