...
static dframetype_t *R_SpriteLoadFrame( model_t *mod, void *pin, mspriteframe_t **ppframe, int num )
{
	...
	if( mod->flags & 256 ) // it's a HUD sprite
	{
		...
		gl_texturenum = GL_LoadTexture( texname, reinterpret_cast<byte*>(pin), pinframe->width * pinframe->height, r_texFlags, NULL );
	}
	else
	{
	...
	pspriteframe = reinterpret_cast<mspriteframe_t*>(Mem_Alloc( mod->mempool, sizeof( mspriteframe_t )));
...
static dframetype_t *R_SpriteLoadGroup( model_t *mod, void *pin, mspriteframe_t **ppframe, int framenum )
{
	...
	pspritegroup = reinterpret_cast<mspritegroup_t*>(Mem_Alloc( mod->mempool, groupsize ));
	pspritegroup->numframes = numframes;

	*ppframe = (mspriteframe_t *)pspritegroup;
	pin_intervals = (dspriteinterval_t *)(pingroup + 1);
	poutintervals = reinterpret_cast<float*>(Mem_Alloc( mod->mempool, numframes * sizeof( float )));
...
void Mod_LoadSpriteModel( model_t *mod, const void *buffer, qboolean *loaded, uint texFlags )
{
	...
	psprite = reinterpret_cast<msprite_t*>(Mem_Alloc( mod->mempool, size ));
...
void Mod_LoadMapSprite( model_t *mod, const void *buffer, size_t size, qboolean *loaded )
{
	...
	pix = reinterpret_cast<rgbdata_t*>(FS_LoadImage( texname, reinterpret_cast<const byte*>(buffer), size ));
	...
	mod->mempool = Mem_AllocPool( va( "^2%s^7", mod->name ));
	psprite = reinterpret_cast<msprite_t*>(Mem_Alloc( mod->mempool, sizeof( msprite_t ) + ( numframes - 1 ) * sizeof( psprite->frames )));
	mod->cache.data = psprite;	// make link to extradata

	psprite->type = SPR_FWD_PARALLEL_ORIENTED;
	psprite->texFormat = SPR_ALPHTEST;
	psprite->numframes = mod->numframes = numframes;
	psprite->radius = (int)sqrtf(static_cast<float>((( w >> 1) * (w >> 1)) + ((h >> 1) * (h >> 1))));
	...
	temp.buffer = reinterpret_cast<byte*>(Mem_Alloc( r_temppool, temp.size ));
	temp.palette = NULL;
	...
	for( i = xl = yl = 0; i < numframes; i++ )
	{
		...
		psprite->frames[i].frameptr = reinterpret_cast<mspriteframe_t*>(Mem_Alloc( mod->mempool, sizeof( mspriteframe_t )));
		...
		pspriteframe->gl_texturenum = GL_LoadTextureInternal( texname, &temp, static_cast<texFlags_t>(TF_IMAGE), false );
...
void Mod_UnloadSpriteModel( model_t *mod )
{
	...
	psprite = reinterpret_cast<msprite_t*>(mod->cache.data);
...
mspriteframe_t *R_GetSpriteFrame( const model_t *pModel, int frame, float yaw )
{
	...
	ASSERT( pModel );
	psprite = reinterpret_cast<msprite_t*>(pModel->cache.data);
...
float R_GetSpriteFrameInterpolant( cl_entity_t *ent, mspriteframe_t **oldframe, mspriteframe_t **curframe )
{
	...
	psprite = reinterpret_cast<msprite_t*>(ent->model->cache.data);
