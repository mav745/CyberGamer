...
void CL_WriteDemoHeader( const char *name )
{
	...
	demo.directory.entries = reinterpret_cast<demoentry_t*>(Mem_Alloc( cls.mempool, sizeof( demoentry_t ) * demo.directory.numentries ));
...
qboolean CL_DemoReadMessage( byte *buffer, size_t *length )
{
	...
	do
	{
		...
		case dem_userdata:
			...
			userbuf = reinterpret_cast<byte*>(Mem_Alloc( cls.mempool, size ));
