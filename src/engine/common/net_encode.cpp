...
void Delta_CustomEncode( delta_info_t *dt, const void *from, const void *to )
{
	...
	if( dt->userCallback )
	{
		dt->userCallback( reinterpret_cast<delta_t*>(dt->pFields), reinterpret_cast<const byte*>(from), reinterpret_cast<const byte*>(to) );
	}
}
...
qboolean Delta_AddField( const char *pStructName, const char *pName, int flags, int bits, float mul, float post_mul )
{
	...
	// allocate a new one
	dt->pFields = reinterpret_cast<delta_t*>(Z_Realloc( dt->pFields, (dt->numFields + 1) * sizeof( delta_t )));
	for( i = 0, pField = dt->pFields; i < dt->numFields; i++, pField++ );
...
void Delta_ParseTable( char **delta_script, delta_info_t *dt, const char *encodeDll, const char *encodeFunc )
{
	...
	{
		dt->pFields = reinterpret_cast<delta_t*>(Z_Realloc( dt->pFields, dt->numFields * sizeof( delta_t )));
	}
...
void Delta_InitFields( void )
{
	...
	afile = reinterpret_cast<char*>(FS_LoadFile( DELTA_PATH, NULL, false ));
...
//		//		  initValue,myValue,iValue,bits,bSigned);
