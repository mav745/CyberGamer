...
void stringlistappend( stringlist_t *list, char *text )
{
	...
	if( list->numstrings >= list->maxstrings )
	{
		...
		list->strings = reinterpret_cast<char**>(Mem_Alloc( fs_mempool, list->maxstrings * sizeof( *list->strings )));
	...
	list->strings[list->numstrings] = reinterpret_cast<char*>(Mem_Alloc( fs_mempool, textlen ));
...
fs_offset_t FS_FileTime( const char *filename, qboolean gamedironly )
{
	...
	if( search->pack ) // grab pack filetime
		return (fs_offset_t)search->pack->filetime;
	else if( search->wad ) // grab wad filetime
		return (fs_offset_t)search->wad->filetime;
	else if( pack_ind < 0 )
	{
		// found in the filesystem?
		char	path [MAX_SYSPATH];
...
search_t *FS_Search( const char *pattern, int caseinsensitive, int gamedironly )
{
	...
	if( resultlist.numstrings )
	{
		...
		search = reinterpret_cast<search_t*>(Mem_Alloc( fs_mempool, sizeof(search_t) + numchars + numfiles * sizeof( char* )));
...
wfile_t *W_Open( const char *filename, const char *mode )
{
	...
	else if( mode[0] == 'r' || mode[0] == 'a' )
	{
		...
		wad->lumps = reinterpret_cast<dlumpinfo_t*>(Mem_Alloc( wad->mempool, wad->numlumps * sizeof( dlumpinfo_t )));
