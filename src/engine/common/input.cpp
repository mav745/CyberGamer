...
long IN_WndProc( void *hWnd, uint uMsg, uint wParam, long lParam )
{
	...
	VGUI_SurfaceWndProc( reinterpret_cast<HWND>(hWnd), uMsg, wParam, lParam );
	...
	case WM_MOUSEWHEEL_V:
		...
		break;
	case WM_CREATE:
		host.hWnd = reinterpret_cast<HWND>(hWnd);
	...
	case WM_MOVE:
		if( !Cvar_VariableInteger( "fullscreen" ))
		{
			...
			style = GetWindowLong( reinterpret_cast<HWND>(hWnd), GWL_STYLE );
