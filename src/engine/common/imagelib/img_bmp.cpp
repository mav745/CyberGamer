...
qboolean Image_LoadBMP( const char *name, const byte *buffer, size_t filesize )
{
	...
	{
		pixbuf = image.palette = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, 1024 ));
...
qboolean Image_SaveBMP( const char *name, rgbdata_t *pix )
{
	...
	if( host.write_to_clipboard )
	{
		...
		clipbuf = reinterpret_cast<byte*>(Z_Malloc( total_size ));
		cur_size = 0;
	}
	else
	{
	...
	pbBmpBits = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, cbBmpBits ));
