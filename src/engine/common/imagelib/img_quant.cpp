...
rgbdata_t *Image_Quantize( rgbdata_t *pic )
{
	...
	image.tempbuffer = reinterpret_cast<byte*>(Mem_Realloc( host.imagepool, image.tempbuffer, image.size ));

	initnet( pic->buffer, pic->size, 10 );
	learn();
	unbiasnet();

	pic->palette = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, netsize * 3 ));
