...
{
	rgbdata_t	*pack = reinterpret_cast<rgbdata_t*>(Mem_Alloc( host.imagepool, sizeof( rgbdata_t )));
...
qboolean FS_AddSideToPack( const char *name, int adjust_flags )
{
	...
	image.cubemap = reinterpret_cast<byte*>(Mem_Realloc( host.imagepool, image.cubemap, image.ptr + image.size ));
...
rgbdata_t *FS_LoadImage( const char *filename, const byte *buffer, size_t size )
{
	...
	for( format = image.loadformats; format && format->formatstring; format++)
	{
		if( anyformat || !Q_stricmp( ext, format->ext ))
		{
			...
			f = FS_LoadFile( path, reinterpret_cast<fs_offset_t*>(&filesize), false );
			if( f && filesize > 0 )
			{
...
rgbdata_t *FS_CopyImage( rgbdata_t *in )
{
	...
	out = reinterpret_cast<rgbdata_t*>(Mem_Alloc( host.imagepool, sizeof( rgbdata_t )));
	*out = *in;
	...
	if( palSize )
	{
		out->palette = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, palSize ));
		Q_memcpy( out->palette, in->palette, palSize );
	}

	if( in->size )
	{
		out->buffer = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, in->size ));
