...
byte *Image_Copy( size_t size )
{
	...
	out = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, size ));
...
void Image_ConvertPalTo24bit( rgbdata_t *pic )
{
	...
	pal24 = converted = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, 768 ));
	pal32 = pic->palette;
...
void Image_CopyPalette32bit( void )
{
	...
	image.palette = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, 1024 ));
...
byte *Image_FlipInternal( const byte *in, word *srcwidth, word *srcheight, int type, int flags )
{
	...
	case PF_RGBA_32:
	case PF_BGRA_32:
		image.tempbuffer = reinterpret_cast<byte*>(Mem_Realloc( host.imagepool, image.tempbuffer, width * height * samples ));
		break;
	default:
...
byte *Image_CreateLumaInternal( byte *fin, int width, int height, int type, int flags )
{
	...
	case PF_INDEXED_24:
	case PF_INDEXED_32:
		out = image.tempbuffer = reinterpret_cast<byte*>(Mem_Realloc( host.imagepool, image.tempbuffer, width * height ));
...
qboolean Image_AddIndexedImageToPack( const byte *in, int width, int height )
{
	...
	image.rgba = reinterpret_cast<byte*>(Mem_Alloc( host.imagepool, image.size ));
...
qboolean Image_Decompress( const byte *data )
{
	...
	image.tempbuffer = reinterpret_cast<byte*>(Mem_Realloc( host.imagepool, image.tempbuffer, size ));
...
rgbdata_t *Image_DecompressInternal( rgbdata_t *pic )
{
	...
	pic->buffer = reinterpret_cast<byte*>(Mem_Realloc( host.imagepool, pic->buffer, image.size ));
