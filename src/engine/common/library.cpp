...
void *MemoryLoadLibrary( const char *name )
{
	...
	CopySections( reinterpret_cast<byte*>(data), old_header, result );
...
qboolean LibraryLoadSymbols( dll_user_t *hInst )
{
	...
	hInst->ordinals = reinterpret_cast<word*>(Mem_Alloc( host.mempool, hInst->num_ordinals * sizeof( word )));
	...
	hInst->funcs = reinterpret_cast<dword*>(Mem_Alloc( host.mempool, hInst->num_ordinals * sizeof( dword )));
	...
	p_Names = reinterpret_cast<dword*>(Mem_Alloc( host.mempool, hInst->num_ordinals * sizeof( dword )));
