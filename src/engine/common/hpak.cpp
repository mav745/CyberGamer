...
void HPAK_AddToQueue( const char *name, resource_t *DirEnt, byte *data, file_t *f )
{
	...
	ptr = reinterpret_cast<hpak_t*>(Z_Malloc( sizeof( hpak_t )));
...
void HPAK_CreatePak( const char *filename, resource_t *DirEnt, byte *data, file_t *f )
{
	...
	if( data == NULL )
	{
		...
		temp = reinterpret_cast<char*>(Z_Malloc( DirEnt->nDownloadSize ));
		FS_Read( f, temp, DirEnt->nDownloadSize );
		FS_Seek( f, filelocation, SEEK_SET );

		MD5Update( &MD5_Hash, reinterpret_cast<byte*>(temp), DirEnt->nDownloadSize );
		Mem_Free( temp );
	}
	else
	{
	...
	MD5Final( reinterpret_cast<byte*>(md5), &MD5_Hash );
	...
	hash_pack_dir.dirs = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t )));
...
void HPAK_AddLump( qboolean add_to_queue, const char *name, resource_t *DirEnt, byte *data, file_t *f )
{
	...
	if( data == NULL )
	{
		// there are better ways
		position = FS_Tell( f );
		temp = reinterpret_cast<byte*>(Z_Malloc( DirEnt->nDownloadSize ));
		FS_Read( f, temp, DirEnt->nDownloadSize );
	...
	// load the data
	hpak1.dirs = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t ) * hpak1.count ));
	FS_Read( f1, hpak1.dirs, sizeof( hpak_dir_t ) * hpak1.count );
	FS_Close( f1 );

	if( HPAK_FindResource( &hpak1, reinterpret_cast<char*>(DirEnt->rgucMD5_hash), NULL ))
	{
		Mem_Free( hpak1.dirs );
		FS_Close( f2 );
	}
	...
	hpak2.dirs = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t ) * hpak2.count ));
...
void HPAK_FlushHostQueue( void )
{
	...
	for( ptr = hpak_queue; ptr != NULL; ptr = hpak_queue )
	{
		...
		HPAK_AddLump( 0, ptr->name, &ptr->HpakResource, reinterpret_cast<byte*>(ptr->data), 0 );
		Mem_Free( ptr->name );
		Mem_Free( ptr->data );
		Mem_Free( ptr );
	}
...
static qboolean HPAK_Validate( const char *filename, qboolean quiet )
{
	...
	dataDir = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t ) * num_lumps ));
	...
	for( i = 0; i < num_lumps; i++ )
	{
		...
		dataPak = reinterpret_cast<byte*>(Z_Malloc( dataDir[i].size ));
...
qboolean HPAK_ResourceForHash( const char *filename, char *inHash, resource_t *pRes )
{
	...
	hpakcontainer.dirs = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t ) * hpakcontainer.count ));
...
qboolean HPAK_GetDataPointer( const char *filename, resource_t *pResource, byte **buffer, int *size )
{
	...
	for( queue = hpak_queue; queue != NULL; queue = queue->next )
	{
		if( !Q_stricmp(queue->name, filename ) && !Q_memcmp( queue->HpakResource.rgucMD5_hash, pResource->rgucMD5_hash, 16 ))
		{
			if( buffer )
			{
				tmpbuf = reinterpret_cast<byte*>(Z_Malloc( queue->size ));
	...
	direntries = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t ) * num_lumps ));
	...
	for( i = 0; i < num_lumps; i++ )
	{
		if( !Q_memcmp( direntries[i].DirectoryResource.rgucMD5_hash, pResource->rgucMD5_hash, 16 ))
		{
			...
			{
				tmpbuf = reinterpret_cast<byte*>(Z_Malloc( direntries[i].size ));
...
void HPAK_RemoveLump( const char *name, resource_t *resource )
{
	...
	hpak_read.dirs = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t ) * hpak_read.count ));
	hpak_save.dirs = reinterpret_cast<hpak_dir_t*>(Z_Malloc( sizeof( hpak_dir_t ) * hpak_save.count ));

	FS_Read( f1, hpak_read.dirs, sizeof( hpak_dir_t ) * hpak_read.count );

	if( !HPAK_FindResource( &hpak_read, reinterpret_cast<char*>(resource->rgucMD5_hash), NULL ))
	{
