...
qboolean Info_SetValueForStarKey( char *s, const char *key, const char *value, int maxsize )
{
	...
	Q_snprintf( new1, sizeof( new1 ) - 1, "\\%s\\%s", key, value );
	if( Q_strlen( new1 ) + Q_strlen( s ) > maxsize )
	{
		MsgDev( D_ERROR, "SetValueForKey: info string length exceeded\n" );
...
char *Cvar_Userinfo( void )
{
	...
	Cvar_LookupVars( CVAR_USERINFO, infostring, NULL, reinterpret_cast<setpair_t>(Cvar_LookupBitInfo) );
	return infostring;
}
...
{
	infostring[0] = 0; // clear previous calls
	Cvar_LookupVars( CVAR_SERVERINFO, infostring, NULL, reinterpret_cast<setpair_t>(Cvar_LookupBitInfo) );
	return infostring;
}
...
{
	infostring[0] = 0; // clear previous calls
	Cvar_LookupVars( CVAR_PHYSICINFO, infostring, NULL, reinterpret_cast<setpair_t>(Cvar_LookupBitInfo) );
	return infostring;
}
