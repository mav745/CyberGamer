...
qboolean NET_GetPacket( netsrc_t sock, netadr_t *from, byte *data, size_t *length )
{
	...
	for( protocol = 0; protocol < 2; protocol++ )
	{
		...
		ret = static_cast<uint>(pRecvFrom( net_socket, reinterpret_cast<char*>(data), NET_MAX_PAYLOAD, 0, (struct sockaddr *)&addr, &addr_len ));
...
void NET_SendPacket( netsrc_t sock, size_t length, const void *data, netadr_t to )
{
	...
	ret = pSendTo( net_socket, reinterpret_cast<const char*>(data), length, 0, &addr, sizeof( addr ));
...
static int NET_IPSocket( const char *netInterface, int port )
{
	...
	if( pBind( net_socket, reinterpret_cast<sockaddr*>(&addr), sizeof( addr )) == SOCKET_ERROR )
	{
...
static int NET_IPXSocket( int port )
{
	...
	if( pIoctlSocket( net_socket, FIONBIO, reinterpret_cast<dword*>(&_true) ) == SOCKET_ERROR )
	{
