...
// mpg123 exports
extern "C" int create_decoder( mpeg_t *mpg );
extern "C" int read_mpeg_header( mpeg_t *mpg, const char *data, long bufsize, long streamsize );
extern "C" int read_mpeg_stream( mpeg_t *mpg, const char *data, long bufsize );
extern "C" int set_current_pos( mpeg_t *mpg, int newpos, int (*pfnSeek)( void*, long, int ), void *file );
extern "C" int get_current_pos( mpeg_t *mpg, int curpos );
extern "C" void close_decoder( mpeg_t *mpg );
...
qboolean Sound_LoadMPG( const char *name, const byte *buffer, size_t filesize )
{
	...
	if( !read_mpeg_header( &mpeg, reinterpret_cast<const char*>(buffer), FRAME_SIZE, filesize ))
	{
...
stream_t *Stream_OpenMPG( const char *filename )
{
	...
	stream = reinterpret_cast<stream_t*>(Mem_Alloc( host.soundpool, sizeof( stream_t )));
	stream->file = file;
	stream->pos = 0;

	mpegFile = reinterpret_cast<mpeg_t*>(Mem_Alloc( host.soundpool, sizeof( mpeg_t )));
...
long Stream_SetPosMPG( stream_t *stream, long newpos )
{
	...
	int newPos = set_current_pos( reinterpret_cast<mpeg_t*>(stream->ptr), newpos, reinterpret_cast<int (*)( void*, long, int )>(FS_Seek), stream->file );
...
{
	return get_current_pos( reinterpret_cast<mpeg_t*>(stream->ptr), stream->pos );
}
