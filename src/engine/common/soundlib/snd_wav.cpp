...
static void FindNextChunk( const char *name )
{
	while( 1 )
	{
		...
		if( !Q_strncmp( reinterpret_cast<const char*>(iff_dataPtr), name, 4 ))
			return;
	}
}
...
qboolean Sound_LoadWAV( const char *name, const byte *buffer, size_t filesize )
{
	...
	if( !( iff_dataPtr && !Q_strncmp( reinterpret_cast<const char*>(iff_dataPtr + 8), "WAVE", 4 )))
	{
	...
	if( iff_dataPtr )
	{
		...
		if( iff_dataPtr )
		{
			if( !Q_strncmp( reinterpret_cast<const char*>(iff_dataPtr + 28), "mark", 4 ))
			{
	...
	sound.wav = reinterpret_cast<byte*>(Mem_Alloc( host.soundpool, sound.size ));
	...
	{
		int	i, j;
		char	*pData = reinterpret_cast<char*>(sound.wav);
