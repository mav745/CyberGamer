...
qboolean Cmd_GetMapList( const char *s, char *completedname, int length )
{
	...
	for( i = 0, nummaps = 0; i < t->numfilenames; i++ )
	{
		...
		{
		case Q1BSP_VERSION:
			if( mapver == 220 ) Q_strncpy( reinterpret_cast<char*>(buf), "Half-Life Alpha", sizeof( buf ));
			else Q_strncpy( reinterpret_cast<char*>(buf), "Quake", sizeof( buf ));
			break;
		case HLBSP_VERSION:
			if( gearbox ) Q_strncpy( reinterpret_cast<char*>(buf), "Blue-Shift", sizeof( buf ));
			else Q_strncpy( reinterpret_cast<char*>(buf), "Half-Life", sizeof( buf ));
			break;
		case XTBSP_VERSION:
			Q_strncpy( reinterpret_cast<char*>(buf), "Xash3D", sizeof( buf ));
			break;
		default:	Q_strncpy( reinterpret_cast<char*>(buf), "??", sizeof( buf )); break;
		}
...
qboolean Cmd_CheckMapsList_R( qboolean fRefresh, qboolean onlyingamedir )
{
	...
	buffer = reinterpret_cast<char*>(Mem_Alloc( host.mempool, t->numfilenames * 2 * sizeof( result )));
...
{
	Cvar_LookupVars( CVAR_ARCHIVE, NULL, f, reinterpret_cast<setpair_t>(Cmd_WriteCvar) );
}

void Cmd_WriteServerVariables( file_t *f )
{
	Cvar_LookupVars( CVAR_SERVERNOTIFY, NULL, f, reinterpret_cast<setpair_t>(Cmd_WriteServerCvar) );
}

void Cmd_WriteOpenGLVariables( file_t *f )
{
	Cvar_LookupVars( CVAR_GLCONFIG, NULL, f, reinterpret_cast<setpair_t>(Cmd_WriteOpenGLCvar) );
}

void Cmd_WriteRenderVariables( file_t *f )
{
	Cvar_LookupVars( CVAR_RENDERINFO, NULL, f, reinterpret_cast<setpair_t>(Cmd_WriteRenderCvar) );
}
...
void Key_EnumCmds_f( void )
{
	...
	if( f )
	{
		...
		FS_Printf( f, "\n\n\t\t\tconsole variables\n\n");
		Cvar_LookupVars( 0, NULL, f, reinterpret_cast<setpair_t>(Cmd_WriteHelp) );
		FS_Printf( f, "\n\n\t\t\tconsole commands\n\n");
		Cmd_LookupCmds( NULL, f, reinterpret_cast<setpair_t>(Cmd_WriteHelp) );
		FS_Printf( f, "\n\n");
		FS_Close( f );
