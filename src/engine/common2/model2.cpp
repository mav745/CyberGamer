...
static void Mod_LoadSubmodels( const dlump_t *l )
{
	...
	// allocate extradata
	out = reinterpret_cast<dmodel_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( *out )));
	...
	if( world.loading )
		world.draw_surfaces = reinterpret_cast<msurface_t**>(Mem_Alloc( loadmodel->mempool, world.max_surfaces * sizeof( msurface_t* )));
}
...
static void Mod_LoadTextures( const dlump_t *l )
{
	...
	in = reinterpret_cast<dmiptexlump_t*>(mod_base + l->fileofs);
	...
	for( i = 0; i < loadmodel->numtextures; i++ )
	{
		...
		if( in->dataofs[i] == -1 )
		{
			...
			tx = reinterpret_cast<texture_t*>(Mem_Alloc( loadmodel->mempool, sizeof( *tx )));
			...
			else
			{
				...
				if( !load_external_luma )
				{
					...
					for( j = wadlist.count - 1; j >= 0; j-- )
					{
						...
						if( FS_FileExists( texpath, false ))
						{
							src = FS_LoadFile( texpath, reinterpret_cast<fs_offset_t*>(&srcSize), false );
							break;
						}
					}
				}
...
static void Mod_LoadTexInfo( const dlump_t *l )
{
	...
	in = reinterpret_cast<dtexinfo_t*>(mod_base + l->fileofs);
	if( l->filelen % sizeof( *in ))
		Host_Error( "Mod_LoadTexInfo: funny lump size in %s\n", loadmodel->name );

	count = l->filelen / sizeof( *in );
		  out = reinterpret_cast<mtexinfo_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( *out )));
...
static void Mod_LoadDeluxemap( void )
{
	...
	in = FS_LoadFile( path, reinterpret_cast<fs_offset_t*>(&world.vecdatasize), true );
	...
	world.deluxedata = reinterpret_cast<color24*>(Mem_Alloc( loadmodel->mempool, world.vecdatasize ));
...
static void Mod_LoadLighting( const dlump_t *l )
{
	...
	in = reinterpret_cast<byte*>(mod_base + l->fileofs);
	...
	case XTBSP_VERSION:
		...
		loadmodel->lightdata = reinterpret_cast<color24*>(Mem_Alloc( loadmodel->mempool, l->filelen ));
...
static void Mod_CalcSurfaceExtents( msurface_t *surf )
{
	...
	for( i = 0; i < 2; i++ )
	{
		bmins[i] = (int)floorf( mins[i] / LM_SAMPLE_SIZE );
		bmaxs[i] = (int)ceilf( maxs[i] / LM_SAMPLE_SIZE );
...
static void Mod_BuildPolygon( mextrasurf_t *info, msurface_t *surf, int numVerts, const float *verts )
{
	...
	buffer = reinterpret_cast<byte*>(Mem_Alloc( loadmodel->mempool, bufSize ));
...
static void Mod_LoadSurfaces( const dlump_t *l )
{
	...
	loadmodel->surfaces = reinterpret_cast<msurface_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( msurface_t )));
	loadmodel->cache.data =  Mem_Alloc( loadmodel->mempool, count * sizeof( mextrasurf_t ));
	out = loadmodel->surfaces;
	info = reinterpret_cast<mextrasurf_t*>(loadmodel->cache.data);
...
static void Mod_LoadVertexes( const dlump_t *l )
{
	...
	out = loadmodel->vertexes = reinterpret_cast<mvertex_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( mvertex_t )));
...
static void Mod_LoadEdges( const dlump_t *l )
{
	...
	count = l->filelen / sizeof( *in );
	loadmodel->edges = out = reinterpret_cast<medge_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( medge_t )));
...
static void Mod_LoadSurfEdges( const dlump_t *l )
{
	...
	in = reinterpret_cast<dsurfedge_t*>( mod_base + l->fileofs );
	if( l->filelen % sizeof( *in ))
		Host_Error( "Mod_LoadSurfEdges: funny lump size in %s\n", loadmodel->name );

	count = l->filelen / sizeof( dsurfedge_t );
	loadmodel->surfedges = reinterpret_cast<int*>(Mem_Alloc( loadmodel->mempool, count * sizeof( dsurfedge_t )));
...
static void Mod_LoadMarkSurfaces( const dlump_t *l )
{
	...
	in = reinterpret_cast<dmarkface_t*>( mod_base + l->fileofs );
	if( l->filelen % sizeof( *in ))
		Host_Error( "Mod_LoadMarkFaces: funny lump size in %s\n", loadmodel->name );

	count = l->filelen / sizeof( *in );
	loadmodel->marksurfaces = out = reinterpret_cast<msurface_t**>(Mem_Alloc( loadmodel->mempool, count * sizeof( *out )));
...
static void Mod_LoadNodes( const dlump_t *l )
{
	...
	out = loadmodel->nodes = reinterpret_cast<mnode_t*>(Mem_Alloc( loadmodel->mempool, loadmodel->numnodes * sizeof( *out )));
...
static void Mod_LoadLeafs( const dlump_t *l )
{
	...
	out = reinterpret_cast<mleaf_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( *out )));
...
static void Mod_LoadPlanes( const dlump_t *l )
{
	...
	out = reinterpret_cast<mplane_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( *out )));
...
static void Mod_LoadVisibility( const dlump_t *l )
{
	...
	loadmodel->visdata = reinterpret_cast<byte*>(Mem_Alloc( loadmodel->mempool, l->filelen ));
...
static void Mod_LoadEntities( const dlump_t *l )
{
	...
	loadmodel->entities = reinterpret_cast<char*>(Mem_Alloc( loadmodel->mempool, l->filelen + 1 ));
...
static void Mod_LoadClipnodes( const dlump_t *l )
{
	...
	count = l->filelen / sizeof( *in );
	out = reinterpret_cast<dclipnode_t*>(Mem_Alloc( loadmodel->mempool, count * sizeof( *out )));
...
static void Mod_LoadClipnodes31( const dlump_t *l, const dlump_t *l2, const dlump_t *l3 )
{
	...
	in2 = reinterpret_cast<dclipnode_t*>(mod_base + l2->fileofs);
	if( l2->filelen % sizeof( *in2 )) Host_Error( "Mod_LoadClipnodes2: funny lump size\n" );
	count2 = l2->filelen / sizeof( *in2 );
	out2 = reinterpret_cast<dclipnode_t*>(Mem_Alloc( loadmodel->mempool, count2 * sizeof( *out2 )));

	in3 = reinterpret_cast<dclipnode_t*>(mod_base + l3->fileofs);
	if( l3->filelen % sizeof( *in3 )) Host_Error( "Mod_LoadClipnodes3: funny lump size\n" );
	count3 = l3->filelen / sizeof( *in3 );
	out3 = reinterpret_cast<dclipnode_t*>(Mem_Alloc( loadmodel->mempool, count3 * sizeof( *out3 )));
...
void Mod_CalcPHS( void )
{
	...
	visofs = reinterpret_cast<int*>(Mem_Alloc( worldmodel->mempool, num * sizeof( int )));
	uncompressed_vis = reinterpret_cast<byte*>(Mem_Alloc( worldmodel->mempool, rowbytes * num * 2 ));
	uncompressed_pas = uncompressed_vis + rowbytes * num;
	compressed_pas = reinterpret_cast<byte*>(Mem_Alloc( worldmodel->mempool, phsdatasize ));
	...
	for( i = 0; i < num; i++, dest += rowwords, scan += rowbytes )
	{
		...
		comp = Mod_CompressVis( reinterpret_cast<byte*>(dest), reinterpret_cast<size_t*>(&rowsize) );
		visofs[i] = vismap_p - vismap; // leaf 0 is a common solid
		total_size += rowsize;

		if( total_size > phsdatasize )
		{
			Host_Error( "CalcPHS: vismap expansion overflow %s > %s\n", Q_memprint( (float)total_size ), Q_memprint( (float)phsdatasize ));
		}
	...
	compressed_pas = reinterpret_cast<byte*>(Mem_Realloc( worldmodel->mempool, compressed_pas, total_size ));
...
void Mod_LoadWorld( const char *name, uint *checksum, qboolean force )
{
	...
	CRC32_MapFile( reinterpret_cast<dword*>(&world.checksum), worldmodel->name );
	world.loading = false;
...
void Mod_LoadCacheFile( const char *filename, cache_user_t *cu )
{
	...
	buf = FS_LoadFile( name, reinterpret_cast<fs_offset_t*>(&size), false );
