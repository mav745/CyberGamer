...
static _inline void Huff_PrepareTree( tree_t tree )
{
	...
	tree[0] = reinterpret_cast<void*>(reinterpret_cast<int>(tree[0])+1);
	//VALUE( tree[0] )++;
...
static _inline void **Huff_GetNode( void **tree )
{
	...
	node = reinterpret_cast<void**>(tree[262]);
	if( !node )
	{
		//value = VALUE( tree[1] )++;
		value = reinterpret_cast<int>(tree[1]);
		tree[1] = reinterpret_cast<void*>(reinterpret_cast<int>(tree[1])+1);
...
static _inline void Huff_Swap( void **tree1, void **tree2, void **tree3 )
{
	...
	a = reinterpret_cast<void**>(tree2[2]);
	if( a )
	{
		if( a[0] == tree2 )
			a[0] = tree3;
	...
	b = reinterpret_cast<void**>(tree3[2]);
...
static _inline void Huff_SwapTrees( void **tree1, void **tree2 )
{
	...
	temp = reinterpret_cast<void**>(tree1[3]);
	tree1[3] = tree2[3];
	tree2[3] = temp;

	temp = reinterpret_cast<void**>(tree1[4]);
	tree1[4] = tree2[4];
	tree2[4] = temp;
	...
	temp = reinterpret_cast<void**>(tree2[3]);
	if( temp ) temp[4] = tree2;

	temp = reinterpret_cast<void**>(tree1[4]);
	if( temp ) temp[3] = tree1;

	temp = reinterpret_cast<void**>(tree2[4]);
...
static void Huff_IncrementFreq_r( void **tree1, void **tree2 )
{
	...
	a = reinterpret_cast<void**>(tree2[3]);
	if( a )
	{
		a = reinterpret_cast<void**>(a[6]);
		if( a == tree2[6] )
		{
			b = reinterpret_cast<void**>(tree2[5]);
			if( b[0] != tree2[2] )
				Huff_Swap( tree1, reinterpret_cast<void**>(b[0]), tree2 );
			Huff_SwapTrees( reinterpret_cast<void**>(b[0]), tree2 );
		}
	}

	a = reinterpret_cast<void**>(tree2[4]);
	if( a && a[6] == tree2[6] )
	{
		b = reinterpret_cast<void**>(tree2[5]);
		b[0] = a;
	...
	{
		a = reinterpret_cast<void**>(tree2[5]);
		a[0] = 0;
		Huff_DeleteNode( tree1, reinterpret_cast<void**>(tree2[5]) );
	}


	//VALUE( tree2[6] )++;
	tree2[6] = reinterpret_cast<void*>(reinterpret_cast<int>(tree2[6])+1);
	a = reinterpret_cast<void**>(tree2[3]);
	if( a && a[6] == tree2[6] )
	{
		tree2[5] = a[5];
	...
	if( tree2[2] )
	{
		Huff_IncrementFreq_r( tree1, reinterpret_cast<void**>(tree2[2]) );

		if( tree2[4] == tree2[2] )
		{
			Huff_SwapTrees( tree2, reinterpret_cast<void**>(tree2[2]) );
			a = reinterpret_cast<void**>(tree2[5]);
...
static void Huff_AddReference( void **tree, int ch )
{
	...
	if( tree[ch + 5] )
	{
		Huff_IncrementFreq_r( tree, reinterpret_cast<void**>(tree[ch + 5]) );
		return; // already added
	}

	//value = VALUE( tree[0] )++;
	value = reinterpret_cast<int>(tree[0]);
	tree[0] = reinterpret_cast<void*>(reinterpret_cast<int>(tree[0])+1);
	b = &tree[value * 8 + 263];
	...
	a[7] = NODE_NEXT;
	a[6] = NODE_START;
	d = reinterpret_cast<void**>(tree[3]);
	a[3] = d[3];
	if( a[3] )
	{
		d = reinterpret_cast<void**>(a[3]);
		d[4] = a;
		d = reinterpret_cast<void**>(a[3]);
		if( d[6] == NODE_START )
		{
	...
	Huff_IncrementFreq_r( tree, reinterpret_cast<void**>(a[2]) );
}
...
{
	if( tree[2] ) Huff_EmitPathToByte( reinterpret_cast<void**>(tree[2]), tree, buffer );
	if( !subtree ) return;
...
static _inline int Huff_GetByteFromTree( void **tree, byte *buffer )
{
	...
	while( tree[7] == NODE_NEXT )
	{
		if( !Huff_GetBit( buffer ))
			tree = reinterpret_cast<void**>(tree[0]);
		else tree = reinterpret_cast<void**>(tree[1]);
		if( !tree ) return 0;
	}
	return VALUE( tree[7] );
}
...
static void Huff_EmitByteDynamic( void **tree, int value, byte *buffer )
{
	...
	subtree = reinterpret_cast<void**>(tree[value + 5]);
	if( subtree )
	{
		if( subtree[2] )
			Huff_EmitPathToByte( reinterpret_cast<void**>(subtree[2]), subtree, buffer );
		return;
	}
...
void Huff_DecompressPacket( sizebuf_t *msg, int offset )
{
	...
	for( i = 0; i < outLen; i++ )
	{
		...
		ch = Huff_GetByteFromTree( reinterpret_cast<void**>(tree[2]), data );
