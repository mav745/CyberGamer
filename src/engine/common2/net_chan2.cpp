...
#define FLOW_INTERVAL		0.1f		// don't compute more often than this
...
void Netchan_OutOfBand( int net_socket, netadr_t adr, int length, byte *data )
{
	...
	{
		// send the datagram
		NET_SendPacket( static_cast<netsrc_t>(net_socket), BF_GetNumBytesWritten( &send ), BF_GetData( &send ), adr );
	}
}
...
void Netchan_OutOfBandPrint( int net_socket, netadr_t adr, char *format, ... )
{
	...
	Netchan_OutOfBand( net_socket, adr, Q_strlen( string ), reinterpret_cast<byte*>(string) );
}
...
void Netchan_UpdateFlow( netchan_t *chan )
{
	...
	for( flow = 0; flow < 2; flow++ )
	{
		...
		if( !faccumulatedtime )
		{
			pflow->kbytespersec = 0.0f;
		}
		else
		{
			pflow->kbytespersec = (float)bytes / faccumulatedtime;
			pflow->kbytespersec /= 1024.0f;
		}

		pflow->avgkbytespersec = ( FLOW_AVG ) * pflow->avgkbytespersec + ( 1.0f - FLOW_AVG ) * pflow->kbytespersec;
	}
}
...
qboolean Netchan_CopyFileFragments( netchan_t *chan, sizebuf_t *msg )
{
	...
	buffer = reinterpret_cast<byte*>(Mem_Alloc( net_mempool, nsize + 1 ));
	p = chan->incomingbufs[ FRAG_FILE_STREAM ];
	pos = 0;
...
void Netchan_TransmitBits( netchan_t *chan, int length, byte *data )
{
	...
	if( chan->sock == NS_CLIENT )
	{
		BF_WriteWord( &send, (int)Cvar_VariableValue( "net_qport" ));
	}
