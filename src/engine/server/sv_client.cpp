...
void SV_BeginRedirect( netadr_t adr, int target, char *buffer, int buffersize, void (*flush))
{
	...
	host.rd.flush = reinterpret_cast<void(*)( netadr_t adr, rdtype_t target, char *buffer )>(flush);
	host.rd.address = adr;
	host.rd.buffer[0] = 0;
}
	...
	else if( type == NETAPI_REQUEST_PLAYERS )
	{
		...
		for( i = 0; i < sv_maxclients->integer; i++ )
		{
			if( svs.clients[i].state >= cs_connected )
			{
				...
				float time = (float)(host.realtime - svs.clients[i].lastconnect);
...
void SV_SendResourceList_f( sv_client_t *cl )
{
	...
	Msg( "Count res: %d\n", rescount );
	Msg( "ResList size: %s\n", Q_memprint( (float)(BF_GetRealBytesWritten( &cl->netchan.message ) - msg_size )));
}
...
void SV_Baselines_f( sv_client_t *cl )
{
	...
	while( BF_GetNumBytesWritten( &cl->netchan.message ) < ( NET_MAX_PAYLOAD / 2 ) && start < svgame.numEntities )
	{
		...
		if( base->number && ( base->modelindex || base->effects != EF_NODRAW ))
		{
			...
			MSG_WriteDeltaEntity( &nullstate, base, &cl->netchan.message, true, SV_IsPlayerIndex( base->number ), (float)sv.time );
		}
		start++;
	}
...
void SV_ExecuteClientMessage( sv_client_t *cl, sizebuf_t *msg )
{
	...
	if(( host.realtime - cl->lastconnect ) < 2.0f && ( frame->latency > 0.0f ))
	{
		frame->latency = 0.0f;
		frame->raw_ping = 0.0f;
	}
