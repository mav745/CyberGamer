...
int SV_CreateCustomization( customization_t *pListHead, resource_t *pResource, int playernumber, int flags, customization_t **pCust, int *nLumps )
{
	...
	pRes = reinterpret_cast<customization_t*>(Z_Malloc( sizeof( customization_t )));
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if( !CustomDecal_Validate( reinterpret_cast<byte*>(pRes->pBuffer), pResource->nDownloadSize ))
				{
					found_problem = true;
				}
				...
				{
					pRes->pInfo = reinterpret_cast<cachewad_t*>(Z_Malloc( sizeof( cachewad_t )));
					pldecal = reinterpret_cast<cachewad_t*>(pRes->pInfo);

					if( pResource->nDownloadSize < 1024 || pResource->nDownloadSize > 16384 )
					{
						found_problem = true;
					}
					else if( !CustomDecal_Init( pldecal, reinterpret_cast<byte*>(pRes->pBuffer), pResource->nDownloadSize, playernumber ))
					{
						found_problem = true;
					}
