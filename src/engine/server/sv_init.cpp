...
char *SV_EntityScript( void )
{
	...
	if( ft2 != -1 )
	{
		...
		else if(( ents = reinterpret_cast<char*>(FS_LoadFile( entfilename, NULL, true ))) != NULL )
		{
...
void SV_InitGame( void )
{
	...
	svs.clients = reinterpret_cast<sv_client_t*>(Z_Malloc( sizeof( sv_client_t ) * sv_maxclients->integer ));
	svs.num_client_entities = sv_maxclients->integer * SV_UPDATE_BACKUP * 64;
	svs.packet_entities = reinterpret_cast<entity_state_t*>(Z_Malloc( sizeof( entity_state_t ) * svs.num_client_entities ));
	svs.baselines = reinterpret_cast<entity_state_t*>(Z_Malloc( sizeof( entity_state_t ) * GI->max_edicts ));
	...
	svgame.globals->deathmatch = (float)Cvar_VariableInteger( "deathmatch" );
	svgame.globals->teamplay = (float)Cvar_VariableInteger( "teamplay" );
	svgame.globals->coop = (float)Cvar_VariableInteger( "coop" );
