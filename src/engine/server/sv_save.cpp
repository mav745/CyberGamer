...
{
	pfnSaveGameComment = reinterpret_cast<void (/*__cdecl*/ *)( char *buffer, int max_length )>(Com_GetProcAddress( svgame.hInstance, "SV_SaveGameComment" ));
}
...
void RestoreSound( soundlist_t *entry )
{
	...
	if( flags & SND_VOLUME ) BF_WriteByte( &sv.signon, (int)(entry->volume * 255) );
	if( flags & SND_ATTENUATION ) BF_WriteByte( &sv.signon, (int)(entry->attenuation * 64) );
...
SAVERESTOREDATA *SV_SaveInit( int size )
{
	...
	pSaveData = reinterpret_cast<SAVERESTOREDATA*>(Mem_Alloc( host.mempool, sizeof(SAVERESTOREDATA) + ( sizeof(ENTITYTABLE) * numents ) + size ));
...
void SV_SaveGameStateGlobals( SAVERESTOREDATA *pSaveData )
{
	...
	// write global data
	header.skillLevel = (int)Cvar_VariableValue( "skill" ); // This is created from an int even though it's a float
	...
	// sky variables
	header.skyColor_r = (int)Cvar_VariableValue( "sv_skycolor_r" );
	header.skyColor_g = (int)Cvar_VariableValue( "sv_skycolor_g" );
	header.skyColor_b = (int)Cvar_VariableValue( "sv_skycolor_b" );
	...
	svgame.dllFuncs.pfnSaveWriteFields( pSaveData, "Save Header", &header, gSaveHeader, VARRAYSIZE( gSaveHeader ));
	pSaveData->time = header.time;

	// write entity table
	for( i = 0; i < pSaveData->tableCount; i++ )
		svgame.dllFuncs.pfnSaveWriteFields( pSaveData, "ETABLE", pSaveData->pTable + i, gEntityTable, VARRAYSIZE( gEntityTable ));

	// write adjacency list
	for( i = 0; i < pSaveData->connectionCount; i++ )
		svgame.dllFuncs.pfnSaveWriteFields( pSaveData, "ADJACENCY", pSaveData->levelList + i, gAdjacency, VARRAYSIZE( gAdjacency ));
	...
	for( i = 0; i < MAX_LIGHTSTYLES; i++ )
	{
		if( sv.lightstyles[i].pattern[0] )
		{
			...
			svgame.dllFuncs.pfnSaveWriteFields( pSaveData, "LIGHTSTYLE", &light, gLightStyle, VARRAYSIZE( gLightStyle ));
		}
	}
}
...
SAVERESTOREDATA *SV_LoadSaveData( const char *level )
{
	...
	pSaveData = reinterpret_cast<SAVERESTOREDATA*>(Mem_Alloc( host.mempool, sizeof(SAVERESTOREDATA) + SumBytes( &sectionsInfo )));
...
void SV_ReadEntityTable( SAVERESTOREDATA *pSaveData )
{
	...
	for( i = 0; i < pSaveData->tableCount; i++ )
		svgame.dllFuncs.pfnSaveReadFields( pSaveData, "ETABLE", pSaveData->pTable + i, gEntityTable, VARRAYSIZE( gEntityTable ));
}
...
void SV_ParseSaveTables( SAVERESTOREDATA *pSaveData, SAVE_HEADER *pHeader, int setupLightstyles )
{
	...
	// process SAVE_HEADER
	svgame.dllFuncs.pfnSaveReadFields( pSaveData, "Save Header", pHeader, gSaveHeader, VARRAYSIZE( gSaveHeader ));
	...
	for( i = 0; i < pSaveData->connectionCount; i++ )
	{
		...
		svgame.dllFuncs.pfnSaveReadFields( pSaveData, "ADJACENCY", pList, gAdjacency, VARRAYSIZE( gAdjacency ));
	}
	...
	{
		svgame.dllFuncs.pfnSaveReadFields( pSaveData, "LIGHTSTYLE", &light, gLightStyle, VARRAYSIZE( gLightStyle ));
...
void SV_SaveClientState( SAVERESTOREDATA *pSaveData, const char *level )
{
	...
	for( i = 0; i < decalCount; i++ )
	{
		...
		decalScale = (word)(entry->scale * 4096);
	...
	if( !svgame.globals->changelevel && S_StreamGetCurrentState( curtrack, looptrack, reinterpret_cast<int*>(&position) ))
	{
		byte	nameSize;
...
SAVERESTOREDATA *SV_SaveGameState( void )
{
	...
	SaveRestore_InitEntityTable( pSaveData, reinterpret_cast<ENTITYTABLE*>(Mem_Alloc( host.mempool, sizeof(ENTITYTABLE) * numents )), numents );
...
int SV_LoadGameState( char const *level, qboolean createPlayers )
{
	...
	Cvar_SetFloat( "skill", (float)header.skillLevel );
	Q_strncpy( sv.name, header.mapName, sizeof( sv.name ));
	svgame.globals->mapname = MAKE_STRING( sv.name );
	Cvar_Set( "sv_skyname", header.skyName );

	// restore sky parms
	Cvar_SetFloat( "sv_skycolor_r", (float)header.skyColor_r );
	Cvar_SetFloat( "sv_skycolor_g", (float)header.skyColor_g );
	Cvar_SetFloat( "sv_skycolor_b", (float)header.skyColor_b );
	...
	// restore serverflags
	svgame.globals->serverflags = (float)header.serverflags;
	...
	for( i = 0; i < pSaveData->tableCount; i++ )
	{
		...
		if( SV_IsValidEdict( pent ) && ( pEntInfo->flags & levelMask )) // screen out the player if he's not to be spawned
		{
			if( pEntInfo->flags & FENTTABLE_GLOBAL )
			{
				...
				svgame.dllFuncs.pfnSaveReadFields( pSaveData, "ENTVARS", &tmpVars, gEntvarsDescription, VARRAYSIZE( gEntvarsDescription ));
...
int SV_SaveGameSlot( const char *pSaveName, const char *pSaveComment )
{
	...
	svgame.dllFuncs.pfnSaveWriteFields( pSaveData, "GameHeader", &gameHeader, gGameHeader, VARRAYSIZE( gGameHeader ));
...
int SV_SaveReadHeader( file_t *pFile, GAME_HEADER *pHeader, int readGlobalState )
{
	...
	pSaveData = reinterpret_cast<SAVERESTOREDATA*>(Mem_Alloc( host.mempool, sizeof( SAVERESTOREDATA ) + tokenSize + size ));
	pSaveData->connectionCount = 0;
	...
	svgame.dllFuncs.pfnSaveReadFields( pSaveData, "GameHeader", pHeader, gGameHeader, VARRAYSIZE( gGameHeader ));
...
qboolean SV_GetComment( const char *savename, char *comment )
{
	...
	if( tokenSize > 0 )
	{
		pTokenList = reinterpret_cast<char**>(Mem_Alloc( host.mempool, tokenCount * sizeof( char* )));
