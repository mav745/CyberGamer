...
{
	svgame.globals->trace_allsolid = static_cast<float>(trace->allsolid);
	svgame.globals->trace_startsolid = static_cast<float>(trace->startsolid);
	svgame.globals->trace_fraction = trace->fraction;
	svgame.globals->trace_plane_dist = trace->plane.dist;
	svgame.globals->trace_flags = 0;
	svgame.globals->trace_inopen = static_cast<float>(trace->inopen);
	svgame.globals->trace_inwater = static_cast<float>(trace->inwater);
...
void SV_CreateDecal( sizebuf_t *msg, const float *origin, int decalIndex, int entityIndex, int modelIndex, int flags, float scale )
{
	...
	BF_WriteWord( msg, (int)scale * 4096 );
}
...
char *SV_ReadEntityScript( const char *filename, int *flags )
{
	...
	*flags |= MAP_IS_EXIST;
	...
	if( ft2 != -1 && ft1 < ft2 )
	{
		...
		ents = reinterpret_cast<char*>(FS_LoadFile( entfilename, NULL, true ));
	}
...
edict_t* pfnFindClientInPVS( edict_t *pEdict )
{
	...
	delta = (float)( sv.time - sv.lastchecktime );
...
static void pfnMakeStatic( edict_t *ent )
{
	...
	clent->frame = (byte)ent->v.frame;
	clent->colormap = ent->v.colormap;
	clent->skin = ent->v.skin;
	clent->rendermode = (byte)ent->v.rendermode;
	clent->renderamt = (byte)ent->v.renderamt;
	clent->rendercolor.r = (byte)ent->v.rendercolor[0];
	clent->rendercolor.g = (byte)ent->v.rendercolor[1];
	clent->rendercolor.b = (byte)ent->v.rendercolor[2];
...
int pfnWalkMove( edict_t *ent, float yaw, float dist, int iMode )
{
	...
	VectorSet( move, cosf( yaw ) * dist, sinf( yaw ) * dist, 0.0f );
...
int SV_BuildSoundMsg( edict_t *ent, int chan, const char *samp, int vol, float attn, int flags, int pitch, const vec3_t pos )
{
	...
	if( flags & SND_ATTENUATION ) BF_WriteByte( &sv.multicast, (int)(attn * 64) );
...
void SV_StartSound( edict_t *ent, int chan, const char *sample, float vol, float attn, int flags, int pitch )
{
	...
	if( flags & SND_VOLUME ) BF_WriteByte( &sv.multicast, (int)(vol * 255) );
	if( flags & SND_ATTENUATION ) BF_WriteByte( &sv.multicast, (int)(attn * 64) );
	if( flags & SND_PITCH ) BF_WriteByte( &sv.multicast, pitch );
...
void pfnParticleEffect( const float *org, const float *dir, float color, float count )
{
	...
	for( i = 0; i < 3; i++ )
	{
		v = bound( -128, (int)(dir[i] * 16.0f), 127 );
		BF_WriteChar( &sv.datagram, v );
	}

	BF_WriteByte( &sv.datagram, (int)count );
	BF_WriteByte( &sv.datagram, (int)color );
...
void pfnBuildSoundMsg( edict_t *pSource, int chan, const char *samp, float fvol, float attn, int fFlags, int pitch, int msg_dest, int msg_type, const float *pOrigin, edict_t *pSend )
{
	...
	SV_BuildSoundMsg( pSource, chan, samp, (int)(fvol * 255), attn, fFlags, pitch, pOrigin );
	pfnMessageEnd();
}
...
void pfnCrosshairAngle( const edict_t *pClient, float pitch, float yaw )
{
	...
	BF_WriteChar( &client->netchan.message, (int)(pitch * 5) );
	BF_WriteChar( &client->netchan.message, (int)(yaw * 5) );
}
...
void pfnGetPlayerStats( const edict_t *pClient, int *ping, int *packet_loss )
{
	...
	if( ping ) *ping = (int)(cl->ping * 1000);	// this is should be cl->latency not ping!
...
{
	reinterpret_cast<int	(*)( char* s )>(pfnPrecacheModel),
	reinterpret_cast<int	(*)( char* s )>(SV_SoundIndex),
	pfnSetModel,
	pfnModelIndex,
	pfnModelFrames,
	pfnSetSize,
	reinterpret_cast<void	(*)( char* s1, char* s2 )>(pfnChangeLevel),
	pfnGetSpawnParms,
	pfnSaveSpawnParms,
	pfnVecToYaw,
	VectorAngles,
	...
	pfnTraceSphere,
	pfnGetAimVector,
	reinterpret_cast<void	(*)( char* str )>(pfnServerCommand),
	Cbuf_Execute,
	pfnClientCommand,
	pfnParticleEffect,
	reinterpret_cast<void	(*)( int style, char* val )>(pfnLightStyle),
	pfnDecalIndex,
	pfnPointContents,
	pfnMessageBegin,
	pfnMessageEnd,
	...
	Cvar_VariableValue,
	reinterpret_cast<const char* (*)( const char *szVarName )>(Cvar_VariableString),
	Cvar_SetFloat,
	Cvar_Set,
	pfnAlertMessage,
	pfnEngineFprintf,
	...
	pfnClientPrintf,
	pfnServerPrint,
	reinterpret_cast<const char *(*)( void )>(Cmd_Args),
	reinterpret_cast<const char *(*)( int argc )>(Cmd_Argv),
	reinterpret_cast<int	(*)( void )>(Cmd_Argc),
	pfnGetAttachment,
	CRC32_Init,
	reinterpret_cast<void	(*)( CRC32_t *pulCRC, void *p, int len )>(CRC32_ProcessBuffer),
	CRC32_ProcessByte,
	pfnCRC32_Final,
	Com_RandomLong,
	Com_RandomFloat,
	...
	pfnTime,
	pfnCrosshairAngle,
	reinterpret_cast<byte*	(*)( char *filename, int *pLength )>(COM_LoadFileForMe),
	COM_FreeFile,
	pfnEndSection,
	reinterpret_cast<int	(*)( char *filename1, char *filename2, int *iCompare )>(COM_CompareFileTime),
	pfnGetGameDir,
	Cvar_RegisterVariable,
	pfnFadeClientVolume,
	pfnSetClientMaxspeed,
	...
	pfnNumberOfEntities,
	pfnGetInfoKeyBuffer,
	reinterpret_cast<char*	(*)( char *infobuffer, char *key )>(Info_ValueForKey),
	reinterpret_cast<void	(*)( char *infobuffer, char *key, char *value )>(Info_SetValueForKey),
	pfnSetClientKeyValue,
	pfnIsMapValid,
	pfnStaticDecal,
	reinterpret_cast<int	(*)( char *s )>(SV_GenericIndex),
	pfnGetPlayerUserId,
	pfnBuildSoundMsg,
	pfnIsDedicatedServer,
	pfnCVarGetPointer,
	pfnGetPlayerWONId,
	reinterpret_cast<void	(*)( char *s, const char *key )>(Info_RemoveKey),
	pfnGetPhysicsKeyValue,
	pfnSetPhysicsKeyValue,
	pfnGetPhysicsInfoString,
	pfnPrecacheEvent,
	SV_PlaybackEventFull,
	reinterpret_cast<unsigned char *(*)( float *org )>(pfnSetFatPVS),
	reinterpret_cast<unsigned char *(*)( float *org )>(pfnSetFatPAS),
	pfnCheckVisibility,
	Delta_SetField,
	Delta_UnsetField,
	Delta_AddEncoder,
	...
	pfnSetGroupMask,
	pfnCreateInstancedBaseline,
	reinterpret_cast<void(*)( struct cvar_s *var, char *value )>(Cvar_DirectSet),
	pfnForceUnmodified,
	pfnGetPlayerStats,
	reinterpret_cast<void(*)( char *cmd_name, void (*function) (void) )>(Cmd_AddGameCommand),
	pfnVoice_GetClientListening,
	pfnVoice_SetClientListening,
	pfnGetPlayerAuthId,
	pfnSequenceGet,
...
qboolean SV_LoadProgs( const char *name )
{
	...
	svgame.globals->maxEntities = GI->max_edicts;
	svgame.globals->maxClients = sv_maxclients->integer;
	svgame.edicts = reinterpret_cast<edict_t*>(Mem_Alloc( svgame.mempool, sizeof( edict_t ) * svgame.globals->maxEntities ));
